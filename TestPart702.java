 ...
 * 抽象类不可以实例化
 * 子类继承抽象类需要实现父类的抽象方法，如果不是先子类设置成抽象类
 * 
 * 接口描述了类的行为是一套规范
 * [public]interface 接口名[extends父接口的名]{
 * 属性必须是常量[public] [static] [final] 属性类型属性名 = 常量值 ;
 * 方法必须是抽象方法[public] [abstract] 返回值类型方法名 ( 参数列表 ) 
 * }
 ...
 * 
 * 面试：抽象类和接口的区别？
 * 1抽象类是个类只能单继承，接口可以实现多个
 * 2抽象类可以有属性，接口只能有常量
 * 3抽象类可以有非抽象方法，接口只能有抽象方法
 * 4抽象类代表实物对象本身，接口代表对象的行为
 */
